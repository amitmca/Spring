...
<body>
<div class="full_w">
	<div class="h_title"><img src="<c:url value='/resources/images/b_edit.png'/>"/>&nbsp;&nbsp;Edit User Details</div>
	<c:if test="${!empty userToEdit}">
       <c:forEach items="${userToEdit}" var="user">
	 <form:form action="/SpringMyBatisWithXML/update" name="addForm" modelAttribute="usr" method="post" onSubmit="return checkInput(this)">
		<div class="element">
			<label for="name"><span class="red">*</span>First Name</label>
			<form:hidden path="userId" cssClass="form_input" value="${user.userId}"/>
			<form:input path="firstName" cssClass="text" onkeydown= "return isAlpha(event.keyCode);" value="${user.firstName}"/>
		</div>
		
		<div class="element">
			<label for="name"><span class="red">*</span>Last Name</label>
			<form:input path="lastName" cssClass="text" onkeydown= "return isAlpha(event.keyCode);" value="${user.lastName}"/>
		</div>
			
		<div class="element">
			<label for="name"><span class="red">*</span>Username</label>
			<form:input path="username" cssClass="text" value="${user.username}"/>
		</div>
		
		<div class="element">
			<label for="name"><span class="red">*</span>Password</label>
			<form:password path="password" cssClass="text" value="${user.password}"/>
		</div>
		
		<div class="element">
			<label for="name"><span class="red">*</span>Phone</label>
			<form:input path="phone" cssClass="text" onkeypress="return isNumberKey(event)" maxlength="10" value="${user.phone}"/>
		</div>
		
		<div class="element">
			<label for="name"><span class="red">*</span>Email Id</label>
			<form:input path="emailId" cssClass="text" value="${user.emailId}"/>
		</div>
	
		<div class="entry">
			<input type="submit" value="Update" name="update" class="add">
		</div>
	</form:form>
	</c:forEach>
   </c:if>
